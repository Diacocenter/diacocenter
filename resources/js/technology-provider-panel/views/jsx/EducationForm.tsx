...
export default function EducationForm() {
    ...
    const countryList = useQuery("countryList", async () => {
        const {data} = await axios.get(route("api.web.v1.technology-provider-panel.country.index"));
    ...
    const postEducation = useMutation((data) => {
        return axios.post(route("api.web.v1.technology-provider-panel.education.store"),data);
    }, {
        onSuccess: data => {
            reset();
            setOpen(true);
