...
@section('content-object')
    ...
    {{--        </form>--}}
    {{--    </div>--}}
    <div class="relative flex flex-col justify-center bg-white shadow rounded-lg container text-center text-black tracking-wider p-8 form-items p-10 h-full">
        <h1 class="text-h2 mb-5 text-darkSteelGray">Recovery Code</h1>
        <p class="my-5 text-body2 font-serif text-lighSteelGray">Enter recovery code that we sent you on your email.</p>
        @if (session('status'))
            <div class="mb-4 font-medium text-sm text-green-600">
                {{ session('status') }}
            </div>
        ...
        <form  method="POST">
            @csrf
            <div class="mb-3">
                <input type="text" class="form-input rounded-lg border border-gray-700 py-2 px-3 w-full"
                       id="exampleInputEmail1"
                       placeholder="Enter code" name="code">
            </div>
            <button type="submit"
                    class="btn btn-primary text-darkSteelGray hover:text-white w-full mt-5 text-xs md:text-sm py-2 rounded-lg">
                Send Recovery
            </button>
        </form>
    </div>
