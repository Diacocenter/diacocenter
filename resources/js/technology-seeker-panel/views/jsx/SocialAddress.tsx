...
import SocialAddressCard from "./SocialAddressCard";
...
export default function SocialAddress() {
    ...
    const socialAddress = useQuery('socialAddress', async () => {
        const {data} = await axios.get(route("api.web.v1.technology-seeker-panel.social-address.index"));
    ...
    const postSocialMedia = useMutation(async (data) => {
        const response = await axios.post(route("api.web.v1.technology-seeker-panel.social-address.store"), data);
