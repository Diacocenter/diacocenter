import {
    ...
    Chip,
    useMediaQuery,
    useTheme, Divider, Snackbar, Alert,
} from "@mui/material";
...
const ContactDetail: FC = () => {
    ...
    const contactDetail = useQuery('contactDetail', async () => {
        const {data} = await axios.get(route("api.web.v1.technology-seeker-panel.account-info.details"));
        contactDetail.data = data.data;
        // if (data.data.profile_picture === "male.png") {
        //     setFiles([])
        // } else {
        //     setFiles([{
        //         source: data.data.profile_picture,
        //         options: {
        //             type: "local"
        //         }
        //     }])
    ...
    const postContactDetails = useMutation(async (data) => {
        const response = await axios.put(route("api.web.v1.technology-seeker-panel.account-info.details.update"), data);
    ...
    const getSlug = useMutation(async (data) => {
        const response = await axios.post(route("api.web.v1.technology-seeker-panel.getSlug"),{slug:data});
        return response.data;
    }, {
        onSuccess: data => {
        }, onError: () => {
