...
const ContactDetail: FC = () => {
    ...
    const contactDetail = useQuery('contactDetail', async () => {
        const {data} = await axios.get(route("api.web.v1.advisor-panel.account-info.details"));
    ...
    const postContactDetails = useMutation(async (data) => {
        const response = await axios.put(route("api.web.v1.advisor-panel.account-info.details.update"), data);
    ...
    const getUploadParams = ({ meta, response }) => {
        return { url: route("api.web.v1.advisor-panel.temporary-file.store"), headers };
    };
